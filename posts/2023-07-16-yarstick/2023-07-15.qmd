...
To maintain consistency with our earlier computations, it's necessary to explicitly set the reference level in the `sensitivity()` and `specificity()` functions, using the `event_level` = "second" argument. This ensures that the factor levels are interpreted in a way that aligns with our initial demonstration.
